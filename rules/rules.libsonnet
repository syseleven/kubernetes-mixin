      ...
      },
      {
        ...
        rules: [
          ...
          },
          {
            ...
            expr: |||
              avg(irate(node_disk_io_time_seconds_total{%(nodeExporterSelector)s,device=~"%(diskDeviceSelector)s"}[1m]))
            ||| % $._config,
          },
          {
            // Disk utilisation (ms spent, by rate() it's bound by 1 second)
            ...
            expr: |||
              avg by (node) (
                irate(node_disk_io_time_seconds_total{%(nodeExporterSelector)s,device=~"%(diskDeviceSelector)s"}[1m])
              * on (namespace, %(podLabel)s) group_left(node)
                node_namespace_pod:kube_pod_info:
              )
            ||| % $._config,
          },
          {
            ...
            expr: |||
              avg(irate(node_disk_io_time_weighted_seconds_total_seconds_total{%(nodeExporterSelector)s,device=~"%(diskDeviceSelector)s"}[1m]) / 1e3)
            ||| % $._config,
          },
          {
            // Disk saturation (ms spent, by rate() it's bound by 1 second)
            ...
            expr: |||
              avg by (node) (
                irate(node_disk_io_time_weighted_seconds_total_seconds_total{%(nodeExporterSelector)s,device=~"%(diskDeviceSelector)s"}[1m]) / 1e3
              * on (namespace, %(podLabel)s) group_left(node)
                node_namespace_pod:kube_pod_info:
              )
            ||| % $._config,
