  ...
  grafanaDashboards+:: {
    ...
    'k8s-node-rsrc-use.json':
      ...
      .addRow(
        ...
        .addPanel(
          g.panel('CPU Utilisation') +
          g.queryPanel('node:node_cpu_utilisation:avg5m{%(clusterLabel)s="$cluster", node="$node"}' % $._config, 'Utilisation') +
  ...
  grafanaDashboards+:: if $._config.showMultiCluster then {
    'k8s-multicluster-rsrc-use.json':
      ...
      .addRow(
        ...
        .addPanel(
          g.panel('CPU Utilisation') +
          g.queryPanel('sum(node:node_cpu_utilisation:avg5m * node:node_num_cpu:sum) by (%(clusterLabel)s) / sum(node:node_num_cpu:sum) by (%(clusterLabel)s)' % $._config, '{{%(clusterLabel)s}}' % $._config, legendLink) +
