  ...
  grafanaDashboards+:: {
    'pods.json':
      ...
      local cpuRow = row.new()
        ...
        .addTarget(prometheus.target(
          'sum by (container_name) (rate(container_cpu_usage_seconds_total{%(cadvisorSelector)s, image!="",container_name!="POD",pod_name="$pod"}[5m]))' % $._config,
      ...
      local networkRow = row.new()
        ...
        .addTarget(prometheus.target(
          'sort_desc(sum by (pod_name) (rate(container_network_receive_bytes_total{%(cadvisorSelector)s, pod_name="$pod"}[5m])))' % $._config,
