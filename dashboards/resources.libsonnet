    ...
    'k8s-resources-cluster.json':
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.statPanel('1 - avg(rate(node_cpu_seconds_total{mode="idle", %(clusterLabel)s="$cluster"}[5m]))' % $._config)
        )
        .addPanel(
  ...
  grafanaDashboards+:: if $._config.showMultiCluster then {
    'k8s-resources-multicluster.json':
      ...
      g.dashboard(
        ...
        .addPanel(
          g.panel('CPU Utilisation') +
          g.statPanel('1 - avg(rate(node_cpu_seconds_total{mode="idle"}[5m]))' % $._config)
        )
        .addPanel(
          g.panel('CPU Requests Commitment') +
